...
const int NUMCITIES = sizeof(Cities) / sizeof(struct city);
...
/* #ifdef _OPENMP
	...
	for( int n = 0;  n < MAXITERATIONS; n++ )
	{
		...
		for( int k = 0; k < NUMCAPITALS; k++ )
		{
			...
			Capitals[k].latsum  = 0.;
			Capitals[k].numsum = 0; */
			Capitals[k].longsum = Cities[k * (NUMCITIES-1) / (NUMCAPITALS-1)].longitude;
			Capitals[k].latsum  = Cities[k * (NUMCITIES-1) / (NUMCAPITALS-1)].latitude;
			Capitals[k].numsum = 1;
		}

		time0 = omp_get_wtime( );

        #pragma omp parallel for default(none), shared(NUMCITIES, Cities, Capitals)
		for( int i = 0; i < NUMCITIES; i++ )
		{
			int capitalnumber = -1;
